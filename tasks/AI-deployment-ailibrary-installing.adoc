...
----
ifeval::["{PROF_DEPLOYMENT}" == "airgapped"]
. On the _remote_ host, download the `SUSE-AI-get-images.sh` script and the `charts.txt` file from the xref:ai-air-gap-stack[air-gap stack].
Edit the list of workloads in the `charts.txt` file to match your preferences and run the script.
+
[source,bash,subs="+attributes"]
----
...
.. Install {kubeflow} as described in xref:kubeflow-installing[].
endif::[]
