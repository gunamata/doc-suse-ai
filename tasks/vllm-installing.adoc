...
----
{prompt_user}helm show values oci://dp.apps.rancher.io/charts/vllm
----
...
----
{prompt_user}kubectl get deployments -n <SUSE_AI_NAMESPACE>
{prompt_user}kubectl delete deployment vllm-deployment-router -n <SUSE_AI_NAMESPACE>
{prompt_user}kubectl delete deployment <MODEL_DEPLOYMENT_NAME> -n <SUSE_AI_NAMESPACE>
----
...
After deleting the deployments, retry the `helm upgrade` command from the beginning of this section.
The {helm} chart will recreate the deployments with the correct labels.
====
